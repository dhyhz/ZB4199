...
with open(r'C:\Users\DHYHZ\Desktop\nbtrial\ingredients.csv') as f:
    ...
    for rec in row1:
        ...
        if rec != []:# and rec[1] == "":
            if rec[0][-1] == ' ':
                if rec[1] != '1':
                    nameset1.add(rec[0][:-1])
            else:
                if rec[1] != '1':
                    nameset1.add(rec[0])
        counter += 1
    for i in sorted(nameset1):
        excipients.append(i)
...
dataset = numpy.zeros(shape=(len(data),len(excipients))) #ingredients

df = pd.DataFrame(dataset,
                       columns = excipients, #ingredients
                       index = data
                       )
...
df.to_csv(r'C:\Users\DHYHZ\Desktop\nbtrial\ematrix_df.csv') #matrix_df.csv')
